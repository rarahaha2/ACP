...
//------------------------------------------------------------------------------
//
// Script:       AcquireImages.js and ~plan-acquire.js  <== DUAL USE
// Author:       Robert B. Denny <rdenny@dc3.com>
// Version:      ===> 8.0.3 <=== CHANGE SCRIPTVERSION BELOW!
// Requires:     ACP 8.0 or later!
//               AcquireSupport 8.0.1 or later
...
// Constants
//
var TSXActivated = true;
var TSX_IL_Succeeded = false;
var OBS_Time;
var currentpath;
var Image_Name;
...
var SCRIPTVERSION = "8.0.3";
var RESUMEPLAN = true;                                                      // Set to false to prevent plans from being resumed where interrupted.
...
function TSXSync()
{
    Console.PrintLine("=== TSXSync JScript By Pluto ===");
    var oImageLink = new ActiveXObject("TheSkyX.ImageLink");
    var oImageLinkResults = new ActiveXObject("TheSkyX.ImageLinkResults");
    //Imaging Process
    ...
    cam.BinX = Bin_Mode;
    cam.BinY = Bin_Mode;
    Console.PrintLine("Taking " + Expose_Time + " sec. exposure, Filter No." + Filter_num + " , Binning = " + Bin_Mode);
    cam.Expose(Expose_Time, 1, Filter_num);
    while (!cam.ImageReady)
    {
        Util.WaitForMilliseconds(1000);
    }
    OBS_Time = (new Date().getTime() - 1556426707950) / 86400000 + 2458601.6979745370 - 2400000.5;
    OBS_Time = OBS_Time.toFixed(6);
    currentpath = new ActiveXObject("Scripting.FileSystemObject").GetFolder(".").Path;
    Image_Name = currentpath + "\\" + OBS_Time + "_" + Expose_Time + "S_" + "Bin" + Bin_Mode + "_" + Object_Name + ".fit";
    if (cam.SaveImage(Image_Name))
    {
        Console.PrintLine(Image_Name + " Expose success!");
    }
    else
    {
        Console.PrintLine(Image_Name + " Expose error!");
    }
    //Image Link Process
    oImageLink.pathToFITS = Image_Name;
	//oImageLink.pathToFITS = "E:\\MarkariansChain_None_0600s_1x1_0003.fit";
    oImageLink.execute();
    if (oImageLinkResults.Succeeded) {
        RA_J2K = oImageLinkResults.ImageCenterRAJ2000;
        Dec_J2K = oImageLinkResults.ImageCenterDecJ2000;
        Console.PrintLine("RA: " + RA_J2K);
        Console.PrintLine("Dec: " + Dec_J2K);
        ImageScale = oImageLinkResults.ImageScale;
        ImagePA = oImageLinkResults.ImagePositionAngle;
        CatalogStarCount = oImageLinkResults.CatalogStarCount;
        ImageStarCount = oImageLinkResults.ImageStarCount;
        SolutionStarCount = oImageLinkResults.SolutionStarCount;
        SolutionRMS = oImageLinkResults.SolutionRMS;
    }
    else
    {
        Console.PrintLine("Error: " + tsx_ilr.ErrorCode + "  " + tsx_ilr.errortext);
    }
    //Sync Process
    if (Util.ScriptTelescope.CanSync)
    {
        Telescope.Tracking = true;
        SUP.SyncToJ2000(RA_J2K, Dec_J2K);
...
function main()
{
    ...
    Sets: for(var iSet = Pl.SetsCompleted; iSet < Pl.Sets; iSet++)          // ==LABEL==
    {
        ...
        if(RESUMEPLAN) Pl.UpdateCompletionState();                          // Update completion status in plan file
        ...
        Targets: for(var iTgt = iTgt0; iTgt < Pl.Targets.length; iTgt++)    // ==LABEL==
        {
            ...
            else                                                            // Slew-to target, check destination limits
            {
                ...
                if(!SafeSlewIf(Tc, "current"))                              // If needed (waited, etc.) Slew now
...
//**BIGDOME+
                        //if(Telescope.CanSync)
                            // Split line causes lint error (typ.)
                        //    curNeedPU = (SUP.EquDist(Tc.RA, Tc.Dec, SUP.LastSyncRAJ2000, SUP.LastSyncDecJ2000) > Prefs.PointingUpdates.MaximumSlew);
                        //else
//**BIGDOME-
                ...
                ImageSets: for(var iImgSet = iImgSet0; iImgSet < Tc.ImageSets.length; iImgSet++)
                {
                    ...
                    if(Prefs.AutoFocus.Enabled && Tc.AutoFocus)
                    {
                        ...
                        if(Telescope.AlignmentMode == 2)                    // GEM only...
                        {
                            ...
                            AutoFlipIf(MAXAFTIME + MAXPUTIME + z, Pl.QuitAt, Tc); // If AF + PU would cross meridian, do autoflip
                        }
                        ...
                        if(!SafeSlewIf(Tc, "current"))                      // Track possible FMO. If fails, next target!
                            break Repeat;
                    ...
                    //
                    // puDone set false at start of target loop...
                    //
                    if(curNeedPU)
                    {
                        if (Tc.NoPointing)
                        ...
                        else
                        {
                            if(Telescope.AlignmentMode == 2)                // GEM only...
                            {
                                ...
                                switch(AutoFlipIf(z, Pl.QuitAt, Tc))
                                {
                            ...
                            if(!puDone)
                            {
                                ...
                                {
                                    TSXSync();
									puDone = TSX_IL_Succeeded;
                                }
                                else
                                {
                                    puDone = SUP.UpdatePointing(Tc.Name, Tc.RA, Tc.Dec, Tc.PA);//PA for Position Angle
                                }
                                isPointing = false;
                            }
                    ...
                    Count: for(var iCnt = iCnt0; iCnt < Is.Count; iCnt++)
                    {
                        ...
                        //
                        // If it's time for periodic or adaptive AF, do that. If auto-flip tests 
                        // above indicate that this should be skipped this time, skip it (skipPerAF).
                        ...
                        if(Prefs.AutoFocus.Enabled)
                        {
                            ...
                            if(afPeriodic || afAdaptive)
                            {
                                ...
                                {
                                    z = Math.max(PREFLIPMARGIN, 0);         // Allow negative PREFLIPMARGIN elsewhere
                                    if(AutoFlipIf(MAXAFTIME + MAXPUTIME + z, Pl.QuitAt, Tc) == AFI_QUITAT)
                                        break Sets;
                                }
                                ...
                                if(!SafeSlewIf(Tc, "current"))              // Time has passed. Re-slew if needed, computing ephemeris.
                        ...
                        {
                            if(AutoFlipIf(Is.Interval + PREFLIPMARGIN, Pl.QuitAt, Tc) == AFI_QUITAT)      // Does pointing update
                        ...
                        try                                                 // Prevent failures from killing script!
                        {
                            ...
                            if(Tn === null) {
                                ...
                                SUP.TakePicture(Is.Interval, Is.Binning, Is.SubFrame, Tc.Dither,
                                            ...
                                            (lastImgOfTgt && slewNext), Tn.Elements, Tn.Name, Tn.RA, 
                                            Tn.Dec, (lastImgOfTgt && rotNext), Tn.PA);
                            }
                        }
                        catch(e)
                        {
                            ...
                            else
                            {
                                if(!finalImgSolveFail)                      // Unless we already failed a final image solve
                                {
                                            ...
                                            60, (!puDone), true))           // 
                                    {
                                        ...
                                        //
                                        // If too far off, recenter. We used to try to skip this if it was the
                                        // last image in the set, but that's too complex now. Tough.
                                        // 
                                        if(SUP.LastSolvePosErr > Prefs.PointingUpdates.MaximumError)
                                        {
                                            if(Tc.Type != Tc.P_MANUAL && !SUP.DoingOffsetTracking)
                                            {
                                                try {
                                                    ...
                                                    if(Telescope.AlignmentMode == 2)  // GEM only...
                                                    {
                                                        switch(AutoFlipIf(0, Pl.QuitAt, Tc))     // Avoid surprise flip in RecenterTarget()
                                                        {
                                                            case AFI_NOFLIP: break;
                                        ...
                                        else if(iCnt < (Is.Count - 1) && Tc.Type != Tc.P_MANUAL) // Don't report on last/only image of a set
                        ...
                        if(SUP.UserActions !== null)
                        {
                            var ret = SUP.UserActions.ImageComplete(imgFile + ".fts");   // Call with full name
...
// all of this, it updates the ephemeris if needed.
//
...
function WaitForRise(Tc, Pl, zd, tMax, TypeStr)
{
    ...
    //
    var sLST = Range24(Tc.RA + Tc.RA - rLST);                               // Setting LST
    var ttSet = Range12(sLST - nLST);                                       // Sidereal time till sets
    if(ttSet < 0.0)                                                         // If already set below ZD
    {
    ...
    if(ttRise <= 0.0) return true;                                          // === RET === Already risen
    //
    ...
    if(t > tNow)                                                            // If not yet risen
    {
        ...
        Console.PrintLine("  (wait for " + Tc.Name + " within " + TypeStr +" at " + t.toUTCTimeString() + ")");
        TrackOffWaitUntil(t);
    }
...
//
function AutoFlipIf(timeNeeded, quitAtTime, Tgt)
{
